...
int main()
{
    InitWindow(800, 600, "Sliders");
    SetTargetFPS(60);
    
    Slider redSlider({300, 150}, {400, 15}, {20, 60});
    Slider greenSlider({300, 250}, {400, 15}, {20, 60});
    Slider blueSlider({300, 350}, {400, 15}, {20, 60});
    
    Vector2 circleCenter = {650, 250};
    ...
    while (!WindowShouldClose())
    {
        ...
        unsigned char red = static_cast<unsigned char>(redSlider.getValue() * 2.55f);
        unsigned char green = static_cast<unsigned char>(greenSlider.getValue() * 2.55f);
        unsigned char blue = static_cast<unsigned char>(blueSlider.getValue() * 2.55f);
        ...
        DrawCircleV(circleCenter, circleRadius, Color{red, green, blue, 255});
        EndDrawing();
    }
    CloseWindow();
}